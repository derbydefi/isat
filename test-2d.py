...
from isat import ISAT # Update import statement to match your codebase

# Define a nonlinear target function
def target_function(input_array):
    return np.sin(input_array) * np.log(input_array + 1)
...
for x in x_test:
    ...
    if nearest_node:
        ...
        approx = nearest_node.data[1][0]  # Extract scalar from numpy array
    else:
        ...
        approx = target_function(np.array([x]))[0]  # Extract scalar from numpy array
